...
func TestDetectArchiveContent(t *testing.T) {
	...
	t.Run("TAR.GZ archive with JSON files", func(t *testing.T) {
		tarContent := createTestTarGz(t, map[string][]byte{
			"queries.json": testutil.SampleFiles["queries_json"].Content,
			"other.txt":    []byte("other content"),
		})

		result := DetectFileType("archive.tar.gz", tarContent)
		// With content-first detection, archives with queries JSON should be detected
		assert.Equal(t, FileTypeQueriesJSON, result)
	})
